...
* */
var motion_shortcode_generator_url = motion_plugin_data.url + 'admin/shortcodes/',
	motionSelectedShortcodeType,
	motion_tb_dialog_helper = {
		motionSelectedShortcodeType: '',
		needsPreview: false,
		setUpButtons: function(){
			var a = this;
			jQuery("#motiondialog-cancel-button").click(function(){
				a.closeDialog()
			});
			jQuery("#motiondialog-insert-button").click(function(){
				a.insertAction()
			});
		},

		setupShortcodeType: function ( shortcode ) {
			motionSelectedShortcodeType = shortcode;
			this.motionSelectedShortcodeType = shortcode;
		},

		loadShortcodeDetails: function() {
			if (motionSelectedShortcodeType) {
				var a = this;
				// Clean out the table rows before applying the new ones.
				jQuery( '#motiondialog-options-table' ).html( '' );
				jQuery.getScript(motion_shortcode_generator_url + "shortcodes/my_" + motionSelectedShortcodeType + ".js", function () {
					a.initializeDialog();

					// Set the default content to the highlighted text, for certain shortcode types.
					switch ( motionSelectedShortcodeType ) {
						case 'box':
						case 'ilink':
						case 'quote':
						case 'button':
		...
		initializeDialog: function (){
			if (typeof frameworkShortcodeAtts == "undefined") {
				jQuery("#motiondialog-shortcode-options").append("<p>"+motion_plugin_data.error_loading_details_for_shortcode+": " + motionSelectedShortcodeType + "</p>");
			} else {
				var a = frameworkShortcodeAtts.attributes,
					b = jQuery("#motiondialog-options-table");
				...
				jQuery('#motiondialog-shortcode-options select option:selected').each( function () {
	...
	makeShortcode: function () {
		...
		jQuery("#motiondialog-options-table td").each(function () {
	...
	insertAction: function () {
		if (typeof frameworkShortcodeAtts != "undefined") {
			var a = this.makeShortcode(),
				name = jQuery('#motiondialog-selected-shortcode').val(),
				columnNum = 1,
				rowNum = 4,
				i = 1;
...
motion_tb_dialog_helper.setUpButtons();
